...
class Villager_sleeping(State):
    ...
    def check_conditions(self):
        if (self.Villager.world.clock_degree < 190
                or self.Villager.world.clock_degree > 250) and (
...
class Villager_Exploring(State):
    ...
    def random_dest(self):
        ...
        self.Villager.destination = Vector2(
            randint(0, w / 12), randint(0, h / 12))

    def do_actions(self):
        if self.Villager.location.get_distance_to(
                self.Villager.destination) < 2.0:
            #This is because it will always be < 1 away,
            #but never get it exactly
            self.random_dest()

    def check_conditions(self):
        if (self.Villager.world.clock_degree < 250.
                and self.Villager.world.clock_degree > 190.) or (
                self.Villager.world.wood >= self.Villager.world.MAXwood):
            return "sleeping"
        ...
        for TREE_pos in self.Villager.world.TreeLocations.itervalues():
            if TREE_pos.get_distance_to(self.Villager.location) < 200:
                ...
                pos = self.Villager.world.get_tile_pos(
                    self.Villager.destination)
...
class Villager_Chopping(State):
    ...
    def check_conditions(self):
        ...
        if self.Villager.world.TileArray[int(pos.x)][int(
                pos.y)].name != "TreePlantedTile_W":
            return "exploring"
        ...
        if self.Villager.location.get_distance_to(
            ...
            new_tile.location = self.Villager.world.get_tile_pos(
                                self.Villager.destination) * 2
            new_tile.rect.topleft = new_tile.location

            self.Villager.world.background.blit(
                new_tile.img, new_tile.location)
            self.Villager.Tile = self.Villager.world.TileArray[int(
                new_tile.location.x / 32)][int(new_tile.location.y / 32)]

            #Removes the old tree from the world.
            if self.Villager.world.remove_tree(self.Villager.Tile.id) is None:
                return "exploring"
...
class Villager_Delivering(State):
    ...
    def entry_actions(self):
        ...
        des = self.Villager.world.get_close_entity(
            "LumberYard", self.Villager.location, 100)
        if des is None:
            des = self.Villager.world.get_close_entity(
                "LumberYard", self.Villager.location, 300)
            if des is None:
