...
class mapGen:
    ...
    def whole_new(self, num_R, size=(256, 256), c1=0, c2=0, c3=0, name=""):
        ...
        w, h = size
        #Very easy to use w and h instead of size[0] and size[1]

        point_list = [point(
            (randint(0, w), randint(0, h))) for i in range(num_R)]
        ...
        surface.lock()
        #Lock because we are setting individual points
        for y in xrange(h):
            #Do row-by-row instead of
            for x in xrange(w):
                #collumn-by-collumn
                currentPoint = point((x, y))
                new_pointlist = sorted(
                    point_list, key=lambda point: point.get_distance(
                    currentPoint))
                    #sort the points in a new list by their distance
                    #from the current point

                currentPoint.brightness = int(
                    c1 * new_pointlist[0].distance + c2 *
                    new_pointlist[1].distance + c3 *
                    new_pointlist[2].distance) / (
                    num_R * (size[0] / ((512.0 / size[0]) * 256)))
                #this uses the coefficients and the new list to find
                #the brightness of the wanted pixel
                #the math at the end is something I came to after
                #about a half hour of fiddling around
                #with values. This will keep the value at a constant
                #and good level, and not flatten anything.

                #maxB = max(maxB, currentPoint.brightness)
                #minB = min(minB, currentPoint.brightness)

                clr = abs(currentPoint.brightness)
                #a lot of the times the values are negative!
                clr = min(255, clr)
                #Make sure we don't for some reason overflow
                surface.set_at((x, y), (clr, clr, clr))
                #set the point
        surface.unlock()
        #unlock because the loop is done

        pygame.display.set_caption("DONE")
        #tell the user that the program has finished calculating
    ...
    def combine_images(self, *pics):
        ...
        num_pics = len(pics)
        w, h = pics[0].get_size()
        #Again, this is very nice for keeping things quick
        surface = pygame.Surface((w, h))
        #Create the surface we will return

        total_map = [[0 for i in xrange(w)] for i in xrange(h)]
        #Create a 2D array to hold all the values
        for PIC in pics:
            #Loops through each of the given pictures
            for y in xrange(h):
                #Row-by-Row
                for x in xrange(w):
                    total_map[x][y] += PIC.get_at((x, y))[0]
                    #Adds the current value at the map to
                    #the corresponding cell on the array

        surface.lock()
        for y in xrange(h):
            #Loop through again, row-by-row
            for x in xrange(w):
                total_map[x][y] /= num_pics
                #average out each cell
                clr = total_map[x][y]
                #use clr so we don't have to use 'total_map[x][y]' 3 times
                surface.set_at((x, y), (clr, clr, clr))
                #set the colors
        surface.unlock()
    ...
    def threshold(self, surface, lower=100, upper=128):
        ...
        #Create a surface with an identical size as the provided image
        new_surface.fill((0, 0, 0))
        #fill with black, we will be setting the white values
        for y in xrange(h):
            #Row-by-Row
            for x in xrange(w):
                if lower <= surface.get_at((x, y))[0] <= upper:
                #Checks to see if the value is in the threshold
                    new_surface.set_at((x, y), (255, 255, 255))
                    #Sets the pixel at that position to white if it is
    ...
    def reallyCoolFull(self, total_size=(256, 256), name="", num_p=25):
        ...
        """
        pic1 = self.whole_new(num_p, total_size, c1=-1, name=name)
        pic2 = self.whole_new(num_p, total_size, c2=1, name=name + ".")
        pic3 = self.whole_new(num_p, total_size, c3=1, name=name + "..")
        pic4 = self.whole_new(
               num_p, total_size, c1=-1, c2=1, name=name + "...")
