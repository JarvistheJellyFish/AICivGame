...
class Lumberjack(GameEntity):
    ...
    def __init__(self, world, img):
        ...
        self.ani = img_func.get_images(
            self.cells, self.times, 1, 1, 0, self.row, self.pic)
...
class Searching(State):
    ...
    def check_conditions(self):
        if self.Lumberjack.location.get_distance_to(
            ...
            #(self.Lumberjack.can_see[1]<<6)+32), 3)
    ...
    def check_conditions(self):
        ...
        if self.Lumberjack.location.get_distance_to(
            ...
            if self.Lumberjack.hit == 4:
                ...
                self.Lumberjack.world.background.blit(
...
class Delivering(State):
    """This state will be used solely to deliver wood from wherever the
    Lumberjack got the wood to the closest Lumber yard. maybe all
    the lumber yards could  be stored in a dictionary similar to trees,
    ...
    def entry_actions(self):
        ...
        des = self.Lumberjack.world.get_close_entity(
            "LumberYard", self.Lumberjack.location, 100)
        if des is None:
            des = self.Lumberjack.world.get_close_entity(
                "LumberYard", self.Lumberjack.location, 300)
            if des is None:
    ...
    def check_conditions(self):
        if self.Lumberjack.location.get_distance_to(
                self.Lumberjack.destination) < 2.0:
