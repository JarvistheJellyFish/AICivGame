...
class Building(GameEntity.GameEntity):
    ...
    def get_images(self, name):
        ...
        try:
            if name != "UC":
                self.basic_img = pygame.image.load(glob.glob(
                    "Images/Buildings/%s.png" % name))
                self.images.append(self.basic_img)
                self.red_img = pygame.image.load(glob.glob(
                    "Images/Buildings/RED_%s.png" % name))
                self.images.append(self.red_img)
                self.dark_img = pygame.image.load(glob.glob(
                    "Images/Buildings/DARK_%s.png" % name))
                self.images.append(self.dark_img)
                self.selected_img = pygame.image.load(glob.glob(
                    "Images/Buildings/SELECTED_%s_Icon" % name))
                self.images.append(self.selected_img)
                self.icon_img = pygame.image.load(glob.glob(
                    "%s_Icon.png" % name))
                self.images.append(self.selected_img)
            else:
                self.house_img = pygame.image.load(
                    "Images/Buildings/UC_House.png")
                self.dock_img = pygame.image.load(
...
class UnderConstruction(Building):
    ...
    def create(self):
        ...
        self.world.add_built(self.will_be, self.world.get_tile_pos(
            self.location + self.world.background_pos) * 32)
        self.world.remove_entity(self)
