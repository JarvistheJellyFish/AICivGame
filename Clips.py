...
class Clips:
    ...
    def __init__(self, world, Osizes):
        ...
        self.minimap = pygame.transform.scale(
            self.world.minimap_img, self.minimap_size)
        ...
        self.rect_view_w = (self.size[0] / self.a) - (
                           (self.size[0] / self.a) / 5)
        self.rect_view_h = self.size[1] / self.b

        self.minimap_rect = pygame.Rect(self.size[0] - self.minimap_size[0],
            self.size[1] - self.minimap_size[1],
    ...
    def render(self, surface, tp, mouse_pos):
        ...
        self.rect_view_pos = ((-1 * self.world.background_pos.x / self.a)
                               + self.size[0] - self.minimap_size[0]
                               + ((self.size[0] / 5) / self.a),
                              (-1 * self.world.background_pos.y / self.b)
                              + self.size[1] - self.minimap_size[1])

        self.rect_view = (self.rect_view_pos, (
            self.rect_view_w, self.rect_view_h))
...
#         surface.blit(self.world.frame_text, (520,2))
        ...
        pygame.draw.rect(surface, (255, 255, 255), (
            0, self.size[1] - 300, self.size[0], 300))
        surface.blit(self.minimap, (self.size[0] - self.minimap_size[0],
            self.size[1] - self.minimap_size[1]))
        pygame.draw.rect(surface, (255, 255, 255), self.rect_view, 1)

        pygame.draw.rect(surface, (0, 0, 0), (self.size[0]
            - self.minimap_size[0], self.size[1] - self.minimap_size[1],
            self.minimap_size[0], self.minimap_size[1]), 2)
        surface.set_clip(None)

    def update_dial(self, surface, tp):
        #Dial goes below here
...
class sidebar():
    ...
    def __init__(self, world, size):
        ...
        for x in range(self.len_tiles_x):
            ...
            for y in range(self.len_tiles_y):
                try:
                    self.tiles[x].append(icon_tile(self.tile_size,
                        buildings[num], (x * self.tile_width,
                        y * self.tile_height)))
    ...
    def render(self, surface, mouse_pos, tp):
        ...
        population = "POPULATION: %d/%d" % (self.world.population,
            self.world.MAXpopulation)
        ...
        rendered_population = self.world.font.render(population, True, (
            255, 255, 255))
        rendered_pos_str = self.world.font.render(pos_str, True, (
            255, 255, 255))

        pygame.draw.rect(surface, self.outline, self.bottom_rect, 4)

        surface.blit(rendered_height, (self.bottom_rect.topleft[0] + 5,
            self.bottom_rect.topleft[1] + 15))
        surface.blit(rendered_name, (self.bottom_rect.topleft[0] + 5,
            self.bottom_rect.topleft[1] + 40))
        surface.blit(rendered_fps, (self.bottom_rect.topleft[0] + 5,
            self.bottom_rect.topleft[1] + 65))
        surface.blit(rendered_food, (self.bottom_rect.topleft[0] + 5,
            self.bottom_rect.topleft[1] + 90))
        surface.blit(rendered_wood, (self.bottom_rect.topleft[0] + 5,
            self.bottom_rect.topleft[1] + 115))
        surface.blit(rendered_population, (self.bottom_rect.topleft[0] + 5,
            self.bottom_rect.topleft[1] + 140))
        surface.blit(rendered_pos_str, (self.bottom_rect.topleft[0] + 5,
            self.bottom_rect.topleft[1] + 165))
